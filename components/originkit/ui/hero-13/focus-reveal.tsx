...
const FocusReveal = ({
  ...
  return (
    <MotionTag aria-label={text} className={className} style={rootStyle}>
      <div className={stackedWords ? "flex flex-col items-center gap-0 sm:gap-1" : "inline-block"}>
        {words.map((word, wordIndex) => {
          const isWhitespace = word.chars.every((c) => /\s/.test(c));
          if (isWhitespace && stackedWords) return null;
          ...
          return (
            ...
            >
              {word.chars.map((char, charOffset) => {
                ...
                const wordText = word.chars.join("").toUpperCase();
                const isOInPeople = char.toUpperCase() === "O" && wordText.includes("PEOPLE");

                return (
                  <motion.span
                    key={`${char}-${index}`}
                    id={isOInPeople ? "hero-o-portal" : undefined}
